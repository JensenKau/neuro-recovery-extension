...
class CnnDynamic2_3(PytorchModel):
    class InternalModel(nn.Module):
        def __init__(
            ...
            dimension3_0 = (22 ** 2) + (22 ** 2) + (dimension1_3 ** 2 * output_chn1_2) + (dimension2_3 ** 2 * output_chn2_2)
        ...
        def forward(self, avg: Tensor, std: Tensor) -> Tensor:
            ...
            joined_out = torch.unsqueeze(torch.cat((torch.flatten(torch.unsqueeze(avg, 1), 1), torch.flatten(avg_out, 1), torch.flatten(torch.unsqueeze(std, 1), 1), torch.flatten(std_out, 1)), 1), 1)
    ...
    def __init__(self) -> None:
        super().__init__("cnn_dynamic_2_3", self.InternalModel)
        self.model = None
        self.params = None
